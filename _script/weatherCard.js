...
const getHourlyWeather = async () => {
	...
	const forecasts =
		data && data.timelines && data.timelines[0] && data.timelines[0].intervals;
	if (forecasts) {
		const hours = forecasts.slice(1, 7).map(hour => ({
			hour: new Date(hour.startTime).toLocaleTimeString('en-us', {
				hour: '2-digit',
			}),
...
const getDailyWeather = async () => {
	const { data } = await fetch(
		`https://api.tomorrow.io/v4/timelines?location=${LOCATION[0]}, ${LOCATION[1]}&fields=temperature&fields=weatherCode&units=imperial&timesteps=1d&apikey=${Keys.weatherKey}`
	).then(response => {
		return response.json();
	});
	const forecasts =
