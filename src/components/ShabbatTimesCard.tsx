import ShabbatTestData from '#/test-data/shabbat'
...
const getServerShabbatData = createServerFn().handler(async () => {
  const parshaResponse = await axios.get(
  ...
  const parsha =
    parshaData.items.find((e) => e.category == 'parashat')?.title || ''
  ...
  for (let i = 0; i < (days || []).length; i++) {
    ...
    const minchaMatches = dayText.match(/\b\d{1,2}:\d{2}(?:am|pm)\s+Mincha\b/gi)
  ...
  return (
    <Card header={parsha}>
      {error ? (
        ...
        times.map((time) => (
          <div key={time.title} className="flex justify-between mx-5 grow py-1">
            <p className="text-sm">{time.title}</p>
            <p className="text-sm text-grey-600">{time.time}</p>
          </div>
        ))
      )}
    </Card>
